...
(function() {
    ...
    try {
        ...
        if (_type && _type !== 'earner') {
            // Has accountType but it's not earner - redirect
...
async function fetchUserData() {
    try {
        ...
        // ✅ Earner-only: double-check after API confirms accountType
        const _accountType = (currentUser.accountType || '').toLowerCase();
        if (_accountType && _accountType !== 'earner') {
            localStorage.clear();
            window.location.href = 'sign-in.html';
            return;
        }
...
async function fetchChartData(period) {
    ...
    try {
        const res = await fetch(`${API_URL}/api/users/earnings-chart?period=${period}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (res.ok) {
            const data = await res.json();
            if (data.success && data.labels && data.data && chartInstance) {
...
function initializeChart() {
    ...
    // ✅ Connect period filter dropdown
...
window.fetchChartData         = fetchChartData;        // ✅ Ad modal dismiss on click
